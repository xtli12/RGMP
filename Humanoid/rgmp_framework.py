...
class RGMPFramework:
    ...
    2. ARGN (Adaptive Recursive Gaussian Network) - processes RGB images using ARGN architecture to predict joint angles
    
    The framework uses ARGN architecture for enhanced spatial attention and geometric understanding.
    """
    ...
    def _load_argn_model(self, model_path):
        """Load trained ARGN model with ARGN architecture"""
        try:
            ...
            print(f"ARGN model with ARGN loaded from {model_path}")
            return model
            
        except Exception as e:
            print(f"Failed to load ARGN model: {e}")
            # Return default ARGN model
    ...
    def _argn_inference(self, observation):
        ...
        # Resize and normalize for ARGN architecture
